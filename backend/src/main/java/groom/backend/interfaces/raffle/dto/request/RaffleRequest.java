...
public class RaffleRequest {
    ...
    @Schema(type = "string", description = "참여 시작 일시", example = "2024-01-01T00:00:00", required = true)
    private LocalDateTime entryStartAt;

    @NotNull
    @JsonFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss")
    @Schema(type = "string", description = "참여 종료 일시", example = "2024-01-31T23:59:59", required = true)
    private LocalDateTime entryEndAt;

    @Schema(type = "string", description = "추첨 일시", example = "2024-02-01T12:00:00", required = true)
    @NotNull
    @JsonFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss")
