...
//    }
    ...
    public ConsumerFactory<String, CouponDelayEvent> couponDelayEventConsumerFactory() {
        ...
        configProps.put(ConsumerConfig.GROUP_ID_CONFIG, "coupon-delay-group");
    ...
    // ===================== 결제 Outbox(payment-events) =====================
    // payload 가 이미 JSON 문자열(Outbox.payload)이라 String-String 으로 단순 발행한다.
    ...
    public ProducerFactory<String, String> paymentEventProducerFactory() {
        ...
        configProps.put(ProducerConfig.ACKS_CONFIG, "all");   // 이벤트 유실 방지 위해 all
        configProps.put(ProducerConfig.RETRIES_CONFIG, 3);
        return new DefaultKafkaProducerFactory<>(configProps);
    }

    @Bean
    public KafkaTemplate<String, String> paymentEventKafkaTemplate() {
    ...
    public ConsumerFactory<String, String> paymentEventConsumerFactory() {
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        configProps.put(ConsumerConfig.GROUP_ID_CONFIG, "payment-event-group");
        configProps.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
